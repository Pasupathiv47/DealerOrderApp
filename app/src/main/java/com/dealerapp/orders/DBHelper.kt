...
class DBHelper(context: Context) : SQLiteOpenHelper(context, "dealer_orders.db", null, 8) {
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        // Safe, idempotent table/column creation regardless of version jump order
        ...
        // Seed default categories only if the table is empty (fresh migration from old version)
    ...
    // Variant Options (master list, tagged by type: ram_storage / size)
    ...
    // Categories (master list, with variant type)
